...
This document is the canonical reference for channel configuration in RedClaw.
...
6. **Stale runtime state**: config changed but `redclaw daemon` was not restarted.

---

## 1. Configuration Namespace

All channel settings live under `channels_config` in `~/.redclaw/config.toml`.
...
When running `redclaw channel start` (or daemon mode), Telegram and Discord now support sender-scoped runtime switching:
...
- Model cache previews come from `redclaw models refresh --provider <ID>`.
- These are runtime chat commands, not CLI subcommands.

## Inbound Image Marker Protocol

RedClaw supports multimodal input through inline message markers:
...
If `[channels_config.matrix]`, `[channels_config.lark]`, or `[channels_config.feishu]` is present but the corresponding feature is not compiled in, `redclaw channel list`, `redclaw channel doctor`, and `redclaw channel start` will report that the channel is intentionally skipped for this build.
...
access_token = "syt_..."
user_id = "@redclaw:matrix.example.com"   # optional, recommended for E2EE
...
port = 6697
nickname = "redclaw-bot"
username = "redclaw"              # optional
channels = ["#redclaw"]
allowed_users = ["*"]
...
- `REDCLAW_LINQ_SIGNING_SECRET` overrides config secret.
...
5. Restart `redclaw daemon` after config changes.
...
```bash
RUST_LOG=info redclaw daemon 2>&1 | tee /tmp/redclaw.log
```

Then filter channel/gateway events:

```bash
rg -n "Matrix|Telegram|Discord|Slack|Mattermost|Signal|WhatsApp|Email|IRC|Lark|DingTalk|QQ|iMessage|Nostr|Webhook|Channel" /tmp/redclaw.log
```
