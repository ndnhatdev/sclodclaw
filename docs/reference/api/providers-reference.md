...
3. Generic fallback env vars: `REDCLAW_API_KEY` then `API_KEY`
...
- After multimodal normalization, RedClaw sends image payloads through Ollama's native `messages[].images` field.
- If a non-vision provider is selected, RedClaw returns a structured capability error instead of silently ignoring images.
...
- RedClaw normalizes a trailing `/api` in `api_url` automatically.
...
- Model discovery: `redclaw models refresh --provider llamacpp`
...
- Model discovery: `redclaw models refresh --provider sglang`
...
- Model discovery: `redclaw models refresh --provider vllm`
...
- Model discovery: `redclaw models refresh --provider osaurus`
...
- Model discovery: `redclaw models refresh --provider nvidia`
