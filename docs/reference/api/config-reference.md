# RedClaw Config Reference (Operator-Oriented)
...
1. `REDCLAW_WORKSPACE` override (if set)
2. persisted `~/.redclaw/active_workspace.toml` marker (if present)
3. default `~/.redclaw/config.toml`

RedClaw logs the resolved config on startup at `INFO` level:

- `Config loaded` with fields: `path`, `workspace`, `source`, `initialized`

Schema export command:

- `redclaw config schema` (prints JSON Schema draft 2020-12 to stdout)
...
- Query runtime traces with:
  - `redclaw doctor traces --limit 20`
  - `redclaw doctor traces --event tool_call_result --contains \"error\"`
  - `redclaw doctor traces --id <trace-id>`
...
1. `REDCLAW_PROVIDER` (explicit override, always wins when non-empty)
...
- Use `REDCLAW_PROVIDER` when you intentionally want runtime env to override a non-default configured provider.
...
- When `enabled = true` and no OTP secret exists, RedClaw generates one and prints an enrollment URI once.
...
| `state_file` | `~/.redclaw/estop-state.json` | Persistent estop state path |
...
- Use CLI command `redclaw estop` to engage and `redclaw estop resume` to clear levels.
...
- Skill loading and `redclaw skills install` both apply a static security audit. Skills that contain symlinks, script-like files, high-risk shell payload snippets, or unsafe markdown link traversal are rejected.
...
3. Validate with `redclaw doctor` before restart/rollout.
...
- `REDCLAW_LINQ_SIGNING_SECRET` overrides `signing_secret` when set.
