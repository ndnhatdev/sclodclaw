...
- CLI `redclaw sop` currently manages definitions only: `list`, `validate`, `show`.
...
2. Create a SOP directory, for example:
   ...
   ```text
   ~/.redclaw/workspace/sops/deploy-prod/SOP.toml
   ~/.redclaw/workspace/sops/deploy-prod/SOP.md
   ```
...
3. Validate and inspect definitions:
   ...
   ```bash
   redclaw sop list
   redclaw sop validate
   redclaw sop show deploy-prod
   ```
