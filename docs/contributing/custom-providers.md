...
RedClaw supports custom API endpoints for both OpenAI-compatible and Anthropic-compatible providers.
...
# or: export REDCLAW_API_KEY="your-api-key"
redclaw agent
```

## llama.cpp Server (Recommended Local Setup)

RedClaw includes a first-class local provider for `llama-server`:
...
```bash
redclaw models refresh --provider llamacpp
redclaw agent -m "hello"
```

You do not need to export `REDCLAW_API_KEY=dummy` for this flow.

## SGLang Server

RedClaw includes a first-class local provider for [SGLang](https://github.com/sgl-project/sglang):
...
```bash
redclaw models refresh --provider sglang
redclaw agent -m "hello"
```
...
RedClaw includes a first-class local provider for [vLLM](https://docs.vllm.ai/):
...
```bash
redclaw models refresh --provider vllm
redclaw agent -m "hello"
```
