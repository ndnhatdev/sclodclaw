# Adding Boards and Tools — RedClaw Hardware Guide

This guide explains how to add new hardware boards and custom tools to RedClaw.

## Quick Start: Add a Board via CLI

```bash
# Add a board (updates ~/.redclaw/config.toml)
redclaw peripheral add nucleo-f401re /dev/ttyACM0
redclaw peripheral add arduino-uno /dev/cu.usbmodem12345
...
With the `rag-pdf` feature, RedClaw can index PDF files:
...
2. **Add to config** — `redclaw peripheral add my-board /dev/ttyUSB0`
...
| Command | Description |
|---------|-------------|
| `redclaw peripheral list` | List configured boards |
| `redclaw peripheral add <board> <path>` | Add board (writes config) |
| `redclaw peripheral flash` | Flash Arduino firmware |
| `redclaw peripheral flash-nucleo` | Flash Nucleo firmware |
| `redclaw hardware discover` | List USB devices |
| `redclaw hardware info` | Chip info via probe-rs |
