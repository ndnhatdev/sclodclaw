...
```bash
# Check config
cat ~/.redclaw/config.toml | grep -A 5 telegram
...
| Health check | <5s | `time redclaw channel doctor` |
...
5. **Monitor logs** for any issues:
   ...
   redclaw daemon
   # or
   RUST_LOG=info redclaw channel start
   ```
...
- Issues: https://github.com/redclaw-labs/redclaw/issues
