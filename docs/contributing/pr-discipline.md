...
Rules for pull request quality, attribution, privacy, and handoff in RedClaw.
...
- If identity-like context is unavoidable, use RedClaw-scoped roles/labels only (e.g., `RedClawAgent`, `RedClawOperator`, `redclaw_user`).
- Recommended identity-safe naming palette:
    - actor labels: `RedClawAgent`, `RedClawOperator`, `RedClawMaintainer`, `redclaw_user`
    - service/runtime labels: `redclaw_bot`, `redclaw_service`, `redclaw_runtime`, `redclaw_node`
    - environment labels: `redclaw_project`, `redclaw_workspace`, `redclaw_channel`
