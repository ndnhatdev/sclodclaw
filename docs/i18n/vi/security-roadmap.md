...
RedClaw đã có **application-layer security xuất sắc**:
...
## So sánh: RedClaw vs PicoClaw vs production grade

| Tính năng | PicoClaw | RedClaw hiện tại | RedClaw + lộ trình | Mục tiêu production |
|---------|----------|--------------|-------------------|-------------------|
...
| Tự kiểm tra bảo mật (`redclaw audit --check`) | 1 ngày | Thấp |
...
[security.audit]
enabled = true
log_path = "~/.config/redclaw/audit.log"
sign_events = true
max_size_mb = 100
...
redclaw security --check
# → ✓ Sandbox: Firejail active
# → ✓ Audit logging enabled (42 events today)
# → → Resource limits: 512MB mem, 50% CPU

# Truy vấn audit log
redclaw audit --user @alice --since 24h
redclaw audit --risk high --violations-only
redclaw audit --verify-signatures

# Kiểm tra sandbox
redclaw sandbox --test
# → Testing isolation...
...
**RedClaw đã an toàn hơn PicoClaw** với:
...
**Khi triển khai lộ trình này**, RedClaw sẽ trở thành:
...
**Giá trị**: biến RedClaw từ "an toàn để kiểm thử" thành "an toàn cho production"
