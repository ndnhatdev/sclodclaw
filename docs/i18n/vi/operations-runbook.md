...
|---|---|---|
| Foreground runtime | `redclaw daemon` | gỡ lỗi cục bộ, phiên ngắn |
| Foreground gateway only | `redclaw gateway` | kiểm thử webhook endpoint |
| User service | `redclaw service install && redclaw service start` | runtime được quản lý liên tục bởi operator |
...
|---|---|---|
| Tính hợp lệ của config | `redclaw doctor` | không có lỗi nghiêm trọng |
| Kết nối channel | `redclaw channel doctor` | các channel đã cấu hình đều khoẻ mạnh |
| Tóm tắt runtime | `redclaw status` | provider/model/channels như mong đợi |
| Heartbeat/trạng thái daemon | `~/.redclaw/daemon_state.json` | file được cập nhật định kỳ |
...
1. Nếu các channel vẫn thất bại, kiểm tra allowlist và thông tin xác thực trong `~/.redclaw/config.toml`.
...
1. sao lưu `~/.redclaw/config.toml`
2. chỉ áp dụng một thay đổi logic tại một thời điểm
3. chạy `redclaw doctor`
