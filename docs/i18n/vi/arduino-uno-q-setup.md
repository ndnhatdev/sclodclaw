# RedClaw trên Arduino Uno Q — Hướng dẫn từng bước

Chạy RedClaw trên phía Linux của Arduino Uno Q. Telegram hoạt động qua WiFi; điều khiển GPIO dùng Bridge (yêu cầu một ứng dụng App Lab tối giản).

---

## Những gì đã có sẵn (Không cần thay đổi code)

RedClaw bao gồm mọi thứ cần thiết cho Arduino Uno Q. **Clone repo và làm theo hướng dẫn này — không cần patch hay code tùy chỉnh nào.**
...
| Setup command | `src/peripherals/uno_q_setup.rs` | `redclaw peripheral setup-uno-q` triển khai Bridge qua scp + arduino-app-cli |
...
## Phase 2: Cài đặt RedClaw trên Uno Q
...
# Clone redclaw (hoặc scp project của bạn)
...
# Cài đặt
sudo cp target/release/redclaw /usr/local/bin/
```
...
# Copy sang Uno Q
scp target/aarch64-unknown-linux-gnu/release/redclaw arduino@<UNO_Q_IP>:~/
ssh arduino@<UNO_Q_IP> "sudo mv ~/redclaw /usr/local/bin/"
```
...
# Cấu hình nhanh
redclaw onboard --api-key YOUR_OPENROUTER_KEY --provider openrouter

# Hoặc tạo config thủ công
mkdir -p ~/.redclaw/workspace
nano ~/.redclaw/config.toml
```
...
**Tại bước này:** Telegram chat hoạt động. Gửi tin nhắn tới bot — RedClaw phản hồi. Chưa có GPIO.

---

## Phase 5: GPIO qua Bridge (RedClaw xử lý tự động)

RedClaw bao gồm Bridge app và setup command.

### 5.1 Triển khai Bridge App

**Từ Mac** (với repo redclaw):
```bash
redclaw peripheral setup-uno-q --host 192.168.0.48
```

**Từ Uno Q** (đã SSH vào):
```bash
redclaw peripheral setup-uno-q
```
...
Giờ khi bạn nhắn tin cho Telegram bot *"Turn on the LED"* hoặc *"Set pin 13 high"*, RedClaw dùng `gpio_write` qua Bridge.
...
| 5 | `git clone https://github.com/redclaw-labs/redclaw.git && cd redclaw` |
| 6 | `cargo build --release --no-default-features` |
| 7 | `redclaw onboard --api-key KEY --provider openrouter` |
| 8 | Chỉnh sửa `~/.redclaw/config.toml` (thêm Telegram bot_token) |
...
- **"command not found: redclaw"** — Dùng đường dẫn đầy đủ: `/usr/local/bin/redclaw` hoặc đảm bảo `~/.cargo/bin` nằm trong PATH.
...
- **Lệnh GPIO bị bỏ qua** — Đảm bảo Bridge app đang chạy (`redclaw peripheral setup-uno-q` triển khai và khởi động nó). Config phải có `board = "arduino-uno-q"` và `transport = "bridge"`.
- **LLM provider (GLM/Zhipu)** — Dùng `default_provider = "glm"` hoặc `"zhipu"` với `GLM_API_KEY` trong env hoặc config. RedClaw dùng endpoint v4 chính xác.
