# Tài liệu tham khảo Providers — RedClaw
...
3. Biến môi trường dự phòng chung: `REDCLAW_API_KEY`, sau đó là `API_KEY`
...
- Sau khi chuẩn hóa multimodal, RedClaw gửi payload hình ảnh qua trường `messages[].images` gốc của Ollama.
- Nếu chọn provider không hỗ trợ vision, RedClaw trả về lỗi rõ ràng thay vì âm thầm bỏ qua hình ảnh.
...
- Khám phá model: `redclaw models refresh --provider nvidia`
