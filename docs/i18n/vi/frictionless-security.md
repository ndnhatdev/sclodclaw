...
[INFO] ✓ Audit logging enabled (~/.config/redclaw/audit.log)
...
$ redclaw security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/redclaw/audit.log
  → 47 events logged today

# Tắt sandbox tường minh (ghi vào config)
$ redclaw config set security.sandbox.enabled false

# Bật backend cụ thể
$ redclaw config set security.sandbox.backend firejail

# Điều chỉnh giới hạn
$ redclaw config set security.resources.max_memory_mb 2048
```
...
```bash
$ redclaw onboard --security-level paranoid
[1/9] Workspace Setup...
...
...
✅ **Hiển thị trạng thái** — `redclaw security --status`
