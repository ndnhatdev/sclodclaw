...
- `cargo check` / `cargo build` dừng lâu ở `Checking redclaw`
...
### Không tìm thấy lệnh `redclaw` sau cài đặt

Triệu chứng:

- Cài đặt thành công nhưng shell không tìm thấy `redclaw`
...
- Dừng các tiến trình `redclaw daemon` / `redclaw channel start` thừa
