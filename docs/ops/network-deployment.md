# Network Deployment — RedClaw on Raspberry Pi and Local Network

This document covers deploying RedClaw on a Raspberry Pi or other host on your local network, with Telegram and optional webhook channels.
...
**Key:** Telegram, Discord, Slack, and Nostr use **outbound connections** — RedClaw connects to external servers/relays. No port forwarding or public IP required.

---

## 2. RedClaw on Raspberry Pi
...
- Telegram channel works: RedClaw polls Telegram API (outbound)
...
```bash
redclaw daemon --host 0.0.0.0 --port 42617
```
...
2. Start a tunnel:
   ...
   ```
   Or use `redclaw tunnel` (see tunnel docs).

3. RedClaw will refuse `0.0.0.0` unless `allow_public_bind = true` or a tunnel is active.
...
- RedClaw calls `https://api.telegram.org/bot{token}/getUpdates`
...
Run `redclaw daemon` — Telegram channel starts automatically.
...
- Keep one runtime instance for the same token (recommended: `redclaw daemon` service).
...
- [ ] Run `redclaw daemon --host 127.0.0.1 --port 42617` (Telegram works without 0.0.0.0)
