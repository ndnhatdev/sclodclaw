...
This page documents supported update and uninstall procedures for RedClaw on macOS (OS X).
...
- Homebrew: `/opt/homebrew/bin/redclaw` (Apple Silicon) or `/usr/local/bin/redclaw` (Intel)
- Cargo/bootstrap/manual: `~/.cargo/bin/redclaw`
...
```bash
redclaw service stop || true
redclaw service uninstall || true
```

Service artifacts removed by `service uninstall`:

- `~/Library/LaunchAgents/com.redclaw.daemon.plist`
...
```bash
brew uninstall redclaw
```

Cargo/bootstrap/manual (`~/.cargo/bin/redclaw`):

```bash
cargo uninstall redclaw || true
rm -f ~/.cargo/bin/redclaw
```
...
```bash
command -v redclaw || echo "redclaw binary not found"
pgrep -fl redclaw || echo "No running redclaw process"
```
