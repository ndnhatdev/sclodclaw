...
| Tình huống | Lệnh |
|----------|---------|
| Có API key, muốn cài nhanh nhất | `redclaw onboard --api-key sk-... --provider openrouter` |
| Muốn được hướng dẫn từng bước | `redclaw onboard --interactive` |
| Đã có config, chỉ cần sửa kênh | `redclaw onboard --channels-only` |
...
- Thiết lập nhanh: `redclaw onboard --api-key "sk-..." --provider openrouter`
- Thiết lập tương tác: `redclaw onboard --interactive`
- Kiểm tra môi trường: `redclaw status` + `redclaw doctor`
