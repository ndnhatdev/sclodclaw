...
RedClaw provides prebuilt binaries for Android devices.
...
The easiest way to run RedClaw on Android is via [Termux](https://termux.dev/).
...
# For 64-bit (aarch64):
curl -LO https://github.com/redclaw-labs/redclaw/releases/latest/download/redclaw-aarch64-linux-android.tar.gz
tar xzf redclaw-aarch64-linux-android.tar.gz

# For 32-bit (armv7):
curl -LO https://github.com/redclaw-labs/redclaw/releases/latest/download/redclaw-armv7-linux-androideabi.tar.gz
tar xzf redclaw-armv7-linux-androideabi.tar.gz
```
...
For advanced users who want to run RedClaw outside Termux:

```bash
# From your computer with ADB
adb push redclaw /data/local/tmp/
adb shell chmod +x /data/local/tmp/redclaw
adb shell /data/local/tmp/redclaw --version
```
