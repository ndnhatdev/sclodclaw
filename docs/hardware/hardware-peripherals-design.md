# Hardware Peripherals Design — RedClaw

RedClaw enables microcontrollers (MCUs) and Single Board Computers (SBCs) to **dynamically interpret natural language commands**, generate hardware-specific code, and execute peripheral interactions in real-time.

## 1. Vision

**Goal:** RedClaw acts as a hardware-aware AI agent that:
...
**Mental model:** RedClaw = brain that understands hardware. Peripherals = arms and legs it controls.
...
RedClaw runs **directly on the device**. The board spins up a gRPC/nanoRPC server and communicates with peripherals locally.
...
2. RedClaw fetches board-specific docs (e.g. ESP32 GPIO mapping)
...
RedClaw runs on the **host** and maintains a hardware-aware link to the target. Used for development, introspection, and flashing.
...
2. RedClaw identifies connected hardware (VID/PID, architecture)
...
2. RedClaw writes/flashes via OpenOCD or probe-rs
3. Confirms success

**Or:**
1. RedClaw auto-discovers: *"STM32 Nucleo on /dev/ttyACM0, ARM Cortex-M4"*
...
|------------------|--------------------------------|----------------------------------|
| RedClaw runs on | Device (ESP32, RPi)           | Host (Mac, Linux)                |
...
Host runs RedClaw; peripheral runs minimal firmware. Simple JSON over serial.

### Mode B: RPi as Host (Native GPIO)

RedClaw on Pi; GPIO via rppal or sysfs. No separate firmware.
...
1. **Startup:** RedClaw loads config, sees `peripherals.boards`.
...
For low-latency, typed RPC between RedClaw and peripherals:
...
- **redclaw-firmware** or **redclaw-peripheral** — a separate crate/workspace.
...
- User flashes this to the board; RedClaw connects and discovers capabilities.

## 9. Implementation Phases

### Phase 1: Skeleton ✅ (Done)

- [x] Add `Peripheral` trait, config schema, CLI (`redclaw peripheral list/add`)
- [x] Add `--peripheral` flag to agent
- [x] Document in AGENTS.md

### Phase 2: Host-Mediated — Hardware Discovery ✅ (Done)

- [x] `redclaw hardware discover`: enumerate USB devices (VID/PID)
- [x] Board registry: map VID/PID → architecture, name (e.g. Nucleo-F401RE)
- [x] `redclaw hardware introspect <path>`: memory map, peripheral list
...
- [x] RedClaw on Raspberry Pi (native GPIO via rppal)
...
- [ ] RedClaw *on* ESP32 (WiFi + LLM, edge-native) — future
...
- Running full RedClaw *on* bare STM32 (no WiFi, limited RAM) — use Host-Mediated instead
...
> *"Boards like ESP, Raspberry Pi, or boards with WiFi can connect to an LLM (Gemini or open-source). RedClaw runs on the device, creates its own gRPC, spins it up, and communicates with peripherals. User asks via WhatsApp: 'move X arm' or 'turn on LED'. RedClaw gets accurate documentation, writes code, executes it, stores it optimally, runs it, and turns on the LED — all on the development board.*
>
> *For STM Nucleo connected via USB/J-Link/Aardvark to my Mac: RedClaw from my Mac accesses the hardware, installs or writes what it wants on the device, and returns the result. Example: 'Hey RedClaw, what are the available/readable addresses on this USB device?' It can figure out what's connected where and suggest."*
