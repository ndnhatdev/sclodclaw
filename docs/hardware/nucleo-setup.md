# RedClaw on Nucleo-F401RE — Step-by-Step Guide

Run RedClaw on your Mac or Linux host. Connect a Nucleo-F401RE via USB. Control GPIO (LED, pins) via Telegram or CLI.

---

## Get Board Info via Telegram (No Firmware Needed)

RedClaw can read chip info from the Nucleo over USB **without flashing any firmware**. Message your Telegram bot:
...
RedClaw includes everything for Nucleo-F401RE:
...
| Flash command | `redclaw peripheral flash-nucleo` | Builds firmware, flashes via probe-rs |
...
### 1.2 Flash via RedClaw

From the redclaw repo root:
...
```bash
redclaw agent --message "Turn on the LED on pin 13"
```
...
| 3 | `redclaw peripheral flash-nucleo` |
| 4 | Add Nucleo to config.toml (path = your serial port) |
| 5 | `redclaw daemon` or `redclaw agent -m "Turn on LED"` |
...
- **GPIO commands ignored** — Check `path` in config matches your serial port. Run `redclaw peripheral list` to verify.
