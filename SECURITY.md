...
2. **GitHub**: Use [GitHub Security Advisories](https://github.com/ndnhatdev/sclodclaw/security/advisories/new)
...
RedClaw implements defense-in-depth security:
...
RedClaw Docker images follow CIS Docker Benchmark best practices:
...
docker inspect --format='{{.Config.User}}' redclaw
# Expected: 65534:65534

# Run with read-only filesystem (production hardening)
docker run --read-only -v /path/to/workspace:/workspace redclaw gateway
```
