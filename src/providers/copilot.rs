...
/// Tokens are cached to `~/.config/redclaw/copilot/` and refreshed
/// automatically.
...
impl CopilotProvider {
    pub fn new(github_token: Option<&str>) -> Self {
        let token_dir = directories::ProjectDirs::from("", "", "redclaw")
            ...
            .unwrap_or_else(|| {
                ...
                std::env::temp_dir().join(format!("redclaw-copilot-{user}"))
            });
