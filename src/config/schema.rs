...
/// Top-level RedClaw configuration, loaded from `config.toml`.
///
/// Resolution order: `REDCLAW_WORKSPACE` env → `active_workspace.toml` marker → `~/.redclaw/config.toml`.
...
pub struct Config {
    ...
    /// API key for the selected provider. Overridden by `REDCLAW_API_KEY` or `API_KEY` env vars.
...
pub enum ProxyScope {
    ...
    Environment,
    /// Apply proxy to all RedClaw-managed HTTP traffic (default).
    #[default]
    Zeroclaw,
...
fn parse_proxy_scope(raw: &str) -> Option<ProxyScope> {
    match raw.trim().to_ascii_lowercase().as_str() {
        ...
        "redhorse" | "internal" | "core" => Some(ProxyScope::Zeroclaw),
...
pub struct QdrantConfig {
    ...
    /// Falls back to `QDRANT_COLLECTION` env var, or default "redhorse_memories".
...
pub struct ObservabilityConfig {
    ...
    /// Service name reported to the OTel collector. Defaults to "redhorse".
    #[serde(default)]
...
/// How RedClaw receives events from Feishu / Lark.
///
...
fn default_estop_state_file() -> String {
    "~/.redclaw/estop-state.json".to_string()
}
...
pub struct AuditConfig {
    ...
    /// Path to audit log file (relative to redhorse dir)
...
impl Default for Config {
    fn default() -> Self {
        ...
        let redclaw_dir = home.join(crate::branding::HOME_DIR);
...
fn default_config_and_workspace_dirs() -> Result<(PathBuf, PathBuf)> {
    crate::config::default_config_and_workspace_dirs()
}
...
async fn load_persisted_workspace_dirs(
    ...
    Ok(Some((config_dir, workspace_dir)))
}
...
pub(crate) async fn persist_active_workspace_config_dir(config_dir: &Path) -> Result<()> {
    ...
    {
        use std::fs::File;
        let dir = File::open(&default_config_dir).with_context(|| {
            format!(
...
impl ConfigResolutionSource {
    const fn as_str(self) -> &'static str {
        match self {
            Self::RedclawConfigDirEnv => "REDCLAW_CONFIG_DIR",
            Self::RedclawWorkspaceEnv => "REDCLAW_WORKSPACE",
...
async fn resolve_runtime_config_dirs(
    ...
    // Precedence 2: REDCLAW_WORKSPACE env
    if let Some(workspace_dir) = env_input.workspace_dir() {
        let workspace_path = PathBuf::from(workspace_dir);
        let (config_dir, workspace_dir) = resolve_config_dir_for_workspace(&workspace_path);
        return Ok((
            config_dir,
            workspace_dir,
            ConfigResolutionSource::RedclawWorkspaceEnv,
        ));
    }

    // Precedence 3: active_workspace.toml marker
...
fn config_dir_creation_error(path: &Path) -> String {
    format!(
        "Failed to create config directory: {}. If running as an OpenRC service, \
         ensure this path is writable by user 'redhorse'.",
        path.display()
    )
}
...
impl Config {
    pub async fn load_or_init() -> Result<Self> {
        let (default_redhorse_dir, default_workspace_dir) = default_config_and_workspace_dirs()?;

        let (redhorse_dir, workspace_dir, resolution_source) =
            resolve_runtime_config_dirs(&default_redhorse_dir, &default_workspace_dir).await?;

        let lifecycle_bootstrap =
            crate::core::lifecycle::ActivationBootstrap::from_config_dir(&redhorse_dir);
        let _modules_lock = lifecycle_bootstrap.load_install_state().await?;

        let config_path = redhorse_dir.join("config.toml");

        fs::create_dir_all(&redhorse_dir)
            .await
            .with_context(|| config_dir_creation_error(&redhorse_dir))?;
        ...
        if config_path.exists() {
            ...
            let store = crate::security::SecretStore::new(&redhorse_dir, config.secrets.encrypt);
    ...
    pub fn apply_env_overrides(&mut self) {
        // API Key: REDCLAW_API_KEY or API_KEY (generic)
        ...
        // 2) REDCLAW_MODEL_PROVIDER/MODEL_PROVIDER (Codex app-server style).
        ...
        if let Ok(provider) = std::env::var("REDCLAW_PROVIDER") {
            ...
            std::env::var("REDCLAW_MODEL_PROVIDER").or_else(|_| std::env::var("MODEL_PROVIDER"))
        {
            if !provider.is_empty() {
                self.default_provider = Some(provider);
            }
        ...
        // Model: REDCLAW_MODEL or MODEL
        if let Ok(model) = std::env::var("REDCLAW_MODEL").or_else(|_| std::env::var("MODEL")) {
            if !model.is_empty() {
                self.default_model = Some(model);
            }
        }

        // Provider HTTP timeout: REDCLAW_PROVIDER_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("REDCLAW_PROVIDER_TIMEOUT_SECS") {
        ...
        // Gateway port: REDCLAW_GATEWAY_PORT or PORT
        if let Ok(port_str) =
            std::env::var("REDCLAW_GATEWAY_PORT").or_else(|_| std::env::var("PORT"))
        {
        ...
        // Gateway host: REDCLAW_GATEWAY_HOST or HOST
        if let Ok(host) = std::env::var("REDCLAW_GATEWAY_HOST").or_else(|_| std::env::var("HOST")) {
            if !host.is_empty() {
                self.gateway.host = host;
            }
        }

        // Allow public bind: REDCLAW_ALLOW_PUBLIC_BIND
        if let Ok(val) = std::env::var("REDCLAW_ALLOW_PUBLIC_BIND") {
            self.gateway.allow_public_bind = val == "1" || val.eq_ignore_ascii_case("true");
        }

        // Temperature: REDCLAW_TEMPERATURE
        if let Ok(temp_str) = std::env::var("REDCLAW_TEMPERATURE") {
            match temp_str.parse::<f64>() {
                ...
                Ok(temp) => {
                    tracing::warn!(
                        "Ignoring REDCLAW_TEMPERATURE={temp}: \
                ...
                Err(_) => {
                    tracing::warn!("Ignoring REDCLAW_TEMPERATURE={temp_str:?}: not a valid number");
                }
            }
        }
        ...
        // Web search enabled: REDCLAW_WEB_SEARCH_ENABLED or WEB_SEARCH_ENABLED
        if let Ok(enabled) = std::env::var("REDCLAW_WEB_SEARCH_ENABLED")
            .or_else(|_| std::env::var("WEB_SEARCH_ENABLED"))
        {
            self.web_search.enabled = enabled == "1" || enabled.eq_ignore_ascii_case("true");
        }

        // Web search provider: REDCLAW_WEB_SEARCH_PROVIDER or WEB_SEARCH_PROVIDER
        if let Ok(provider) = std::env::var("REDCLAW_WEB_SEARCH_PROVIDER")
        ...
        // Brave API key: REDCLAW_BRAVE_API_KEY or BRAVE_API_KEY
        if let Ok(api_key) =
            std::env::var("REDCLAW_BRAVE_API_KEY").or_else(|_| std::env::var("BRAVE_API_KEY"))
        {
        ...
        // Web search max results: REDCLAW_WEB_SEARCH_MAX_RESULTS or WEB_SEARCH_MAX_RESULTS
        if let Ok(max_results) = std::env::var("REDCLAW_WEB_SEARCH_MAX_RESULTS")
        ...
        // Web search timeout: REDCLAW_WEB_SEARCH_TIMEOUT_SECS or WEB_SEARCH_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("REDCLAW_WEB_SEARCH_TIMEOUT_SECS")
        ...
        // Storage provider key (optional backend override): REDCLAW_STORAGE_PROVIDER
        if let Ok(provider) = std::env::var("REDCLAW_STORAGE_PROVIDER") {
            let provider = provider.trim();
            if !provider.is_empty() {
        ...
        // Storage connection URL (for remote backends): REDCLAW_STORAGE_DB_URL
        if let Ok(db_url) = std::env::var("REDCLAW_STORAGE_DB_URL") {
        ...
        // Storage connect timeout: REDCLAW_STORAGE_CONNECT_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("REDCLAW_STORAGE_CONNECT_TIMEOUT_SECS") {
            if let Ok(timeout_secs) = timeout_secs.parse::<u64>() {
                if timeout_secs > 0 {
        ...
        // Proxy enabled flag: REDCLAW_PROXY_ENABLED
        let explicit_proxy_enabled = std::env::var("REDCLAW_PROXY_ENABLED")
            .ok()
            .as_deref()
        ...
        // Proxy URLs: REDCLAW_* wins, then generic *PROXY vars.
        let mut proxy_url_overridden = false;
        if let Ok(proxy_url) =
            std::env::var("REDCLAW_HTTP_PROXY").or_else(|_| std::env::var("HTTP_PROXY"))
        {
            self.proxy.http_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Ok(proxy_url) =
            std::env::var("REDCLAW_HTTPS_PROXY").or_else(|_| std::env::var("HTTPS_PROXY"))
        {
            self.proxy.https_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Ok(proxy_url) =
            std::env::var("REDCLAW_ALL_PROXY").or_else(|_| std::env::var("ALL_PROXY"))
        {
            self.proxy.all_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Ok(no_proxy) =
            std::env::var("REDCLAW_NO_PROXY").or_else(|_| std::env::var("NO_PROXY"))
        {
        ...
        if let Ok(scope_raw) = std::env::var("REDCLAW_PROXY_SCOPE") {
            if let Some(scope) = parse_proxy_scope(&scope_raw) {
                self.proxy.scope = scope;
            } else {
                tracing::warn!(
                    scope = %scope_raw,
                    "Ignoring invalid REDCLAW_PROXY_SCOPE (valid: environment|redhorse|services)"
                );
            }
        }

        if let Ok(services_raw) = std::env::var("REDCLAW_PROXY_SERVICES") {
    ...
    pub async fn save(&self) -> Result<()> {
        ...
        let redhorse_dir = self
            .config_path
            .parent()
            .context("Config path must have a parent directory")?;
        let store = crate::security::SecretStore::new(redhorse_dir, self.secrets.encrypt);
...
mod tests {
    ...
    async fn config_dir_creation_error_mentions_openrc_and_path() {
        let msg = config_dir_creation_error(Path::new("/etc/redhorse"));
        assert!(msg.contains("/etc/redhorse"));
        assert!(msg.contains("OpenRC"));
        assert!(msg.contains("redhorse"));
    }
...
"#;
    ...
    async fn config_save_and_load_tmpdir() {
        let dir = std::env::temp_dir().join("redhorse_test_config");
        let _ = fs::remove_dir_all(&dir).await;
        fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn config_save_encrypts_nested_credentials() {
        let dir = std::env::temp_dir().join(format!(
            "redhorse_test_nested_credentials_{}",
            uuid::Uuid::new_v4()
        ));
        fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn config_save_atomic_cleanup() {
        let dir =
            std::env::temp_dir().join(format!("redhorse_test_config_{}", uuid::Uuid::new_v4()));
        fs::create_dir_all(&dir).await.unwrap();
...
"#;
    ...
    async fn whatsapp_config_backend_type_cloud_precedence_when_ambiguous() {
        let wc = WhatsAppConfig {
            ...
            verify_token: Some("ver".into()),
            app_secret: None,
            session_path: Some("~/.redclaw/state/whatsapp-web/session.db".into()),
            pair_phone: None,
            pair_code: None,
            allowed_numbers: vec!["+1".into()],
        };
...
"#;
    ...
    fn clear_proxy_env_test_vars() {
        for key in [
            "REDCLAW_PROXY_ENABLED",
            "REDCLAW_HTTP_PROXY",
            "REDCLAW_HTTPS_PROXY",
            "REDCLAW_ALL_PROXY",
            "REDCLAW_NO_PROXY",
            "REDCLAW_PROXY_SCOPE",
            "REDCLAW_PROXY_SERVICES",
            "HTTP_PROXY",
            "HTTPS_PROXY",
            "ALL_PROXY",
            "NO_PROXY",
    ...
    async fn env_override_api_key() {
        ...
        std::env::set_var("REDCLAW_API_KEY", "sk-test-env-key");
        config.apply_env_overrides();
        assert_eq!(config.api_key.as_deref(), Some("sk-test-env-key"));

        std::env::remove_var("REDCLAW_API_KEY");
    }
    ...
    async fn env_override_provider() {
        ...
        std::env::set_var("REDCLAW_PROVIDER", "anthropic");
        config.apply_env_overrides();
        assert_eq!(config.default_provider.as_deref(), Some("anthropic"));

        std::env::remove_var("REDCLAW_PROVIDER");
    }
    ...
    async fn env_override_model_provider_alias() {
        ...
        std::env::remove_var("REDCLAW_PROVIDER");
        std::env::set_var("REDCLAW_MODEL_PROVIDER", "openai-codex");
        config.apply_env_overrides();
        assert_eq!(config.default_provider.as_deref(), Some("openai-codex"));

        std::env::remove_var("REDCLAW_MODEL_PROVIDER");
    }
...
"#;
    ...
    async fn env_override_open_skills_enabled_and_dir() {
        ...
        std::env::set_var("REDCLAW_OPEN_SKILLS_ENABLED", "true");
        std::env::set_var("REDCLAW_OPEN_SKILLS_DIR", "/tmp/open-skills");
        std::env::set_var("REDCLAW_SKILLS_PROMPT_MODE", "compact");
        config.apply_env_overrides();
        ...
        std::env::remove_var("REDCLAW_OPEN_SKILLS_ENABLED");
        std::env::remove_var("REDCLAW_OPEN_SKILLS_DIR");
        std::env::remove_var("REDCLAW_SKILLS_PROMPT_MODE");
    }
    ...
    async fn env_override_open_skills_enabled_invalid_value_keeps_existing_value() {
        ...
        std::env::set_var("REDCLAW_OPEN_SKILLS_ENABLED", "maybe");
        std::env::set_var("REDCLAW_SKILLS_PROMPT_MODE", "invalid");
        config.apply_env_overrides();
    ...
    async fn env_override_zero_claw_provider_overrides_non_default_provider() {
        ...
        std::env::set_var("REDCLAW_PROVIDER", "openrouter");
    ...
    async fn env_override_model() {
        ...
        std::env::set_var("REDCLAW_MODEL", "gpt-4o");
        config.apply_env_overrides();
        assert_eq!(config.default_model.as_deref(), Some("gpt-4o"));

        std::env::remove_var("REDCLAW_MODEL");
    }
    ...
    async fn resolve_runtime_config_dirs_uses_env_workspace_first() {
        ...
        std::env::set_var("REDCLAW_WORKSPACE", &workspace_dir);
        let (config_dir, resolved_workspace_dir, source) =
            resolve_runtime_config_dirs(&default_config_dir, &default_workspace_dir)
                .await
                .unwrap();

        assert_eq!(source, ConfigResolutionSource::RedclawWorkspaceEnv);
    ...
    async fn resolve_runtime_config_dirs_uses_env_config_dir_first() {
        ...
        std::env::set_var("REDCLAW_CONFIG_DIR", &explicit_config_dir);
        std::env::remove_var("REDCLAW_WORKSPACE");
        ...
        assert_eq!(source, ConfigResolutionSource::RedclawConfigDirEnv);
        ...
        std::env::remove_var("REDCLAW_CONFIG_DIR");
        let _ = fs::remove_dir_all(default_config_dir).await;
    }
    ...
    async fn load_or_init_workspace_override_keeps_existing_legacy_config() {
        ...
        let legacy_config_dir = temp_home.join(".redhorse");
...
"#,
    ...
    async fn env_override_empty_values_ignored() {
        ...
        std::env::set_var("REDCLAW_PROVIDER", "");
        config.apply_env_overrides();
    ...
    async fn env_override_gateway_port() {
        ...
        std::env::set_var("REDCLAW_GATEWAY_PORT", "8080");
        config.apply_env_overrides();
        assert_eq!(config.gateway.port, 8080);

        std::env::remove_var("REDCLAW_GATEWAY_PORT");
    }
    ...
    async fn env_override_gateway_host() {
        ...
        std::env::set_var("REDCLAW_GATEWAY_HOST", "0.0.0.0");
        config.apply_env_overrides();
        assert_eq!(config.gateway.host, "0.0.0.0");

        std::env::remove_var("REDCLAW_GATEWAY_HOST");
    }
    ...
    async fn env_override_temperature() {
        ...
        std::env::set_var("REDCLAW_TEMPERATURE", "0.5");
        config.apply_env_overrides();
        assert!((config.default_temperature - 0.5).abs() < f64::EPSILON);

        std::env::remove_var("REDCLAW_TEMPERATURE");
    }
    ...
    async fn env_override_temperature_out_of_range_ignored() {
        ...
        std::env::set_var("REDCLAW_TEMPERATURE", "3.0");
        config.apply_env_overrides();
        assert!(
    ...
    async fn env_override_reasoning_enabled() {
        ...
        std::env::set_var("REDCLAW_REASONING_ENABLED", "false");
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(false));

        std::env::set_var("REDCLAW_REASONING_ENABLED", "true");
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(true));

        std::env::remove_var("REDCLAW_REASONING_ENABLED");
    }
    ...
    async fn env_override_reasoning_invalid_value_ignored() {
        ...
        std::env::set_var("REDCLAW_REASONING_ENABLED", "maybe");
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(false));
    ...
    async fn env_override_storage_provider_config() {
        ...
        std::env::set_var("REDCLAW_STORAGE_PROVIDER", "postgres");
        std::env::set_var("REDCLAW_STORAGE_DB_URL", "postgres://example/db");
        std::env::set_var("REDCLAW_STORAGE_CONNECT_TIMEOUT_SECS", "15");
        ...
        std::env::remove_var("REDCLAW_STORAGE_PROVIDER");
        std::env::remove_var("REDCLAW_STORAGE_DB_URL");
        std::env::remove_var("REDCLAW_STORAGE_CONNECT_TIMEOUT_SECS");
    }
    ...
    async fn env_override_proxy_scope_services() {
        ...
        let mut config = Config::default();
        std::env::set_var("REDCLAW_PROXY_ENABLED", "true");
        std::env::set_var("REDCLAW_HTTP_PROXY", "http://127.0.0.1:7890");
        std::env::set_var(
            "REDCLAW_PROXY_SERVICES",
            "provider.openai, tool.http_request",
        );
        std::env::set_var("REDCLAW_PROXY_SCOPE", "services");
    ...
    async fn env_override_proxy_scope_environment_applies_process_env() {
        ...
        let mut config = Config::default();
        std::env::set_var("REDCLAW_PROXY_ENABLED", "true");
        std::env::set_var("REDCLAW_PROXY_SCOPE", "environment");
        std::env::set_var("REDCLAW_HTTP_PROXY", "http://127.0.0.1:7890");
        std::env::set_var("REDCLAW_HTTPS_PROXY", "http://127.0.0.1:7891");
        std::env::set_var("REDCLAW_NO_PROXY", "localhost,127.0.0.1");
...
[security.estop]
enabled = true
state_file = "~/.redclaw/estop-state.json"
require_otp_to_resume = true
"#,
        )
        .unwrap();
