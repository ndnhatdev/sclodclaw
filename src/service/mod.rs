...
fn check_redhorse_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "redhorse"]).output();
    let is_alpine = Path::new("/etc/alpine-release").exists();

    let (del_cmd, add_cmd) = if is_alpine {
        (
            "deluser redhorse && delgroup redhorse",
            "addgroup -S redhorse && adduser -S -s /sbin/nologin -H -D -G redhorse redhorse",
        )
    } else {
        ("userdel redhorse", "useradd -r -s /sbin/nologin redhorse")
    };
        ...
        Ok(output) if output.status.success() => {
            ...
            if parts.len() >= 7 {
                ...
                if uid.parse::<u32>().unwrap_or(999) >= 1000 {
                    bail!(
                        "User 'redhorse' exists but has unexpected UID {} (expected system UID < 1000).\n\
                ...
                if !shell.contains("nologin") && !shell.contains("false") {
                    bail!(
                        "User 'redhorse' exists but has unexpected shell '{}'.\n\
...
fn ensure_redhorse_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "redhorse"]).output();
    if let Ok(output) = output {
        if output.status.success() {
            return check_redhorse_user();
        }
    }
    ...
    if is_alpine {
        let group_output = Command::new("getent").args(["group", "redhorse"]).output();
        ...
        if !group_exists {
            let output = Command::new("addgroup")
                .args(["-S", "redhorse"])
                .output()
                .context("Failed to create redhorse group")?;

            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                bail!("Failed to create redhorse group: {}", stderr.trim());
            }
            println!("✅ Created system group: redhorse");
        }
        ...
        let output = Command::new("adduser")
            ...
            .output()
            .context("Failed to create redhorse user")?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            bail!("Failed to create redhorse user: {}", stderr.trim());
        }
    } else {
        let output = Command::new("useradd")
            .args(["-r", "-s", "/sbin/nologin", "redhorse"])
            .output()
            .context("Failed to create redhorse user")?;
    ...
    println!("✅ Created system user: redhorse");
    Ok(())
}

/// Change ownership of a path to redhorse:redhorse
#[cfg(unix)]
fn chown_to_redhorse(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["redhorse:redhorse", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to change ownership of {} to redhorse:redhorse: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_to_redhorse(_path: &Path) -> Result<()> {
    Ok(())
}

#[cfg(unix)]
fn chown_recursive_to_redhorse(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["-R", "redhorse:redhorse", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to recursively change ownership of {} to redhorse:redhorse: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_recursive_to_redhorse(_path: &Path) -> Result<()> {
    Ok(())
}
...
fn install_linux_openrc(config: &Config) -> Result<()> {
    ...
    if !workspace_dir.exists() {
        ...
        chown_to_redhorse(&workspace_dir)?;
        println!(
            "✅ Created directory: {} (owned by redhorse:redhorse)",
    ...
    chown_recursive_to_redhorse(config_dir)?;
...
mod tests {
    ...
    fn openrc_writability_probe_prefers_runuser_when_available() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/redhorse"), true);
    ...
    fn openrc_writability_probe_falls_back_to_su() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/redhorse/workspace"), false);
