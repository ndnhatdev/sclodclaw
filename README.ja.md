...
| 日付 (UTC) | レベル | お知らせ | 対応 |
|---|---|---|---|
| 2026-02-19 | _緊急_ | 私たちは `openagen/redclaw` および `redclaw.org` とは**一切関係ありません**。`redclaw.org` は現在 `openagen/redclaw` の fork を指しており、そのドメイン/リポジトリは当プロジェクトの公式サイト・公式プロジェクトを装っています。 | これらの情報源による案内、バイナリ、資金調達情報、公式発表は信頼しないでください。必ず[本リポジトリ](https://github.com/redclaw-labs/redclaw)と認証済み公式SNSのみを参照してください。 |
| 2026-02-21 | _重要_ | 公式サイトを公開しました: [redclawlabs.ai](https://redclawlabs.ai)。公開までお待ちいただきありがとうございました。引き続きなりすましの試みを確認しているため、RedClaw 名義の投資・資金調達などの案内は、公式チャネルで確認できない限り参加しないでください。 | 情報は[本リポジトリ](https://github.com/redclaw-labs/redclaw)を最優先で確認し、[X（@redclawlabs）](https://x.com/redclawlabs?s=21)、[Telegram（@redclawlabs）](https://t.me/redclawlabs)、[Facebook（グループ）](https://www.facebook.com/groups/redclaw)、[Reddit（r/redclawlabs）](https://www.reddit.com/r/redclawlabs/) と [小紅書アカウント](https://www.xiaohongshu.com/user/profile/67cbfc43000000000d008307?xsec_token=AB73VnYnGNx5y36EtnnZfGmAmS-6Wzv8WMuGpfwfkg6Yc%3D&xsec_source=pc_search) で公式更新を確認してください。 |
...
RedClaw は、高速・省リソース・高拡張性を重視した自律エージェント実行基盤です。RedClawはエージェントワークフローのための**ランタイムオペレーティングシステム**です — モデル、ツール、メモリ、実行を抽象化し、エージェントを一度構築すればどこでも実行できるインフラストラクチャです。
...
## ベンチマークスナップショット（RedClaw vs OpenClaw、再現可能）
...
- Release バイナリ: `8.8M`
- `redclaw --help`: 約 `0.02s`、ピークメモリ 約 `3.9MB`
- `redclaw status`: 約 `0.01s`、ピークメモリ 約 `4.1MB`
...
RedClaw はサブスクリプションベースのネイティブ認証プロファイルをサポートしています（マルチアカウント対応、保存時暗号化）。

- 保存先: `~/.redclaw/auth-profiles.json`
- 暗号化キー: `~/.redclaw/.secret_key`
...
<p align="center">
  <img src="docs/assets/architecture.svg" alt="RedClaw アーキテクチャ" width="900" />
</p>
...
未対応の `runtime.kind` が設定された場合、RedClaw は native へのサイレントフォールバックではなく、明確なエラーで終了します。
