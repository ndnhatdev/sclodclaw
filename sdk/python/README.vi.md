...
Gói Python đồng hành cho [RedClaw](https://github.com/redclaw-labs/redclaw) — gọi công cụ dựa trên LangGraph cho thực thi agent LLM nhất quán.
...
```bash
pip install redclaw-tools[discord]
```
...
# Chạy CLI
redclaw-tools "List files in the current directory"
...
```python
from redclaw_tools import tool
...
## So sánh với Rust RedClaw

| Tính năng | Rust RedClaw | redclaw-tools |
|---------|---------------|----------------|
...
Dùng **Rust RedClaw** cho triển khai biên (edge) trong sản phẩm. Dùng **redclaw-tools** khi cần đảm bảo tính nhất quán gọi công cụ hoặc tích hợp hệ sinh thái Python.
