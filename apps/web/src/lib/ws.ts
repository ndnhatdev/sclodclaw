...
export type WsMessageHandler = (msg: ClientEvent) => void;
...
const SESSION_STORAGE_KEY = 'redclaw_session_id';
...
export class WebSocketClient {
  ...
  connect(): void {
    ...
    const url = `${this.baseUrl}${WS_CHAT_PATH}?${params.toString()}`;
    ...
    this.ws.onmessage = (ev: MessageEvent) => {
      try {
        const msg = JSON.parse(ev.data) as ClientEvent;
        this.onMessage?.(msg);
      } catch {
  ...
  sendMessage(content: string): void {
    ...
    this.ws.send(JSON.stringify(createMessageCommand(content)));
  }
