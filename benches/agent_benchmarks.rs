//! Performance benchmarks for RedClaw hot paths.
//!
//! Benchmarks cover:
...
//! Run: `cargo bench`
//!
//! Ref: https://github.com/redclaw-labs/redclaw/issues/618 (item 7)
...
use redhorse::agent::dispatcher::{NativeToolDispatcher, ToolDispatcher, XmlToolDispatcher};
use redhorse::config::MemoryConfig;
use redhorse::memory;
use redhorse::memory::{Memory, MemoryCategory};
...
<tool_call>
{"name": "search", "arguments": {"query": "redclaw architecture"}}
</tool_call>
    ...
    let response = ChatResponse {
        ...
        tool_calls: vec![
            ToolCall {
                id: "tc1".into(),
                name: "search".into(),
                arguments: r#"{"query": "redclaw"}"#.into(),
            },
            ToolCall {
                id: "tc2".into(),
                name: "read_file".into(),
...
fn bench_memory_operations(c: &mut Criterion) {
    ...
    rt.block_on(async {
        for i in 0..100 {
            mem.store(
                &format!("key_{i}"),
                &format!("Content entry number {i} about redclaw agent runtime"),
                MemoryCategory::Core,
                None,
            )
            .await
    ...
    c.bench_function("memory_recall_top10", |b| {
        b.iter(|| {
            rt.block_on(async {
                mem.recall(black_box("redclaw agent"), 10, None)
                    .await
                    .unwrap()
            })
        });
