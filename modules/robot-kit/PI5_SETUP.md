...
Complete guide to setting up a RedClaw-powered robot on Raspberry Pi 5.
...
mkdir -p ~/.redclaw/models
cp models/ggml-base.bin ~/.redclaw/models/
```
...
mkdir -p ~/.redclaw/models/piper
cd ~/.redclaw/models/piper
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx.json

# Test
echo "Hello, I am your robot!" | piper --model ~/.redclaw/models/piper/en_US-lessac-medium.onnx --output_file test.wav
aplay test.wav
```
...
### 7. Build RedClaw Robot Kit

```bash
# Clone repo (or copy from USB)
git clone https://github.com/redclaw-labs/redclaw
cd redclaw

# Build robot kit
cargo build --release -p redclaw-robot-kit

# Build main redclaw (optional, if using as agent)
cargo build --release
```
...
```bash
mkdir -p ~/.redclaw
nano ~/.redclaw/robot.toml
```

```toml
# ~/.redclaw/robot.toml - Real Hardware Configuration
...
# Test speaker
echo "Testing speaker" | piper --model ~/.redclaw/models/piper/en_US-lessac-medium.onnx --output_file - | aplay -D plughw:0,0
...
FIFO_PATH = "/tmp/redclaw_sensors.fifo"
...
```bash
# Configure RedClaw to use robot tools
cat > ~/.redclaw/config.toml << 'EOF'
...
cp ~/redclaw/modules/robot-kit/SOUL.md ~/.redclaw/workspace/

# Start agent
./target/release/redclaw agent
```
...
# Start sensor loop
if [ ! -p /tmp/redclaw_sensors.fifo ]; then
    mkfifo /tmp/redclaw_sensors.fifo
fi
...
```bash
# /etc/systemd/system/redclaw-robot.service
sudo tee /etc/systemd/system/redclaw-robot.service << 'EOF'
[Unit]
Description=RedClaw Robot
After=network.target ollama.service

[Service]
Type=simple
User=pi
WorkingDirectory=/home/pi/redclaw
...
sudo systemctl enable redclaw-robot
sudo systemctl start redclaw-robot

# Check status
sudo systemctl status redclaw-robot
journalctl -u redclaw-robot -f  # View logs
```
