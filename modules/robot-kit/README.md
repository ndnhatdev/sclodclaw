...
A complete toolkit for building AI-powered robots with RedClaw. Designed for Raspberry Pi deployment with offline Ollama inference.
...
### 1. Build RedClaw with robot tools

```bash
# Clone and build
git clone https://github.com/redclaw-labs/redclaw
cd redclaw
cargo build -p redclaw-robot-kit --release
```
...
# Copy config
mkdir -p ~/.redclaw
cp modules/robot-kit/robot.toml ~/.redclaw/
cp modules/robot-kit/SOUL.md ~/.redclaw/workspace/
```
...
# Test in mock mode
./target/release/redclaw agent -m "Say hello and show a happy face"

# Test with real hardware
# (after configuring robot.toml)
./target/release/redclaw agent -m "Move forward 1 meter"
```
...
```rust
use redclaw_robot_kit::{create_tools, RobotConfig};
...
If you want runtime registration in `redclaw`, add a thin adapter that maps this
...
mkdir redclaw-robot-kit
cp -r target/release/redclaw redclaw-robot-kit/
cp -r examples/robot_kit redclaw-robot-kit/
cp -r ~/.redclaw redclaw-robot-kit/dot-redclaw

# Include models
mkdir -p redclaw-robot-kit/models
cp ~/.redclaw/models/ggml-base.bin redclaw-robot-kit/models/
# Note: Ollama models are large, may want to download on target

# Create tarball
tar -czvf redclaw-robot-kit.tar.gz redclaw-robot-kit/

# Copy to USB
cp redclaw-robot-kit.tar.gz /media/usb/TarBalls/
```
