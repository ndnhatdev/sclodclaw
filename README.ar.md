...
<p align="center" dir="rtl">
  RedClaw هو <strong>نظام تشغيل وقت التشغيل</strong> لعمليات العمل الآلية — بنية تحتية تجرد النماذج والأدوات والذاكرة والتنفيذ لبناء وكلاء مرة واحدة وتشغيلهم في أي مكان.
</p>
...
| ---------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _حرج_  | **نحن غير مرتبطين** بـ `openagen/redclaw` أو `redclaw.org`. نطاق `redclaw.org` يشير حاليًا إلى الفرع `openagen/redclaw`، وهذا النطاق/المستودع ينتحل شخصية موقعنا/مشروعنا الرسمي.                                                                                                                                                                                 | لا تثق بالمعلومات أو الملفات الثنائية أو جمع التبرعات أو الإعلانات من هذه المصادر. استخدم فقط [هذا المستودع](https://github.com/redclaw-labs/redclaw) وحساباتنا الموثقة على وسائل التواصل الاجتماعي.                                                                                                                                                                                                                                                                                                                                                          |
| 2026-02-21 | _مهم_ | موقعنا الرسمي أصبح متاحًا الآن: [redclawlabs.ai](https://redclawlabs.ai). شكرًا لصبرك أثناء الانتظار. لا نزال نكتشف محاولات الانتحال: لا تشارك في أي نشاط استثمار/تمويل باسم RedClaw إذا لم يتم نشره عبر قنواتنا الرسمية.                                                                                                                   | استخدم [هذا المستودع](https://github.com/redclaw-labs/redclaw) كمصدر وحيد للحقيقة. تابع [X (@redclawlabs)](https://x.com/redclawlabs?s=21)، [Telegram (@redclawlabs)](https://t.me/redclawlabs)، [Facebook (مجموعة)](https://www.facebook.com/groups/redclaw)، [Reddit (r/redclawlabs)](https://www.reddit.com/r/redclawlabs/)، و[Xiaohongshu](https://www.xiaohongshu.com/user/profile/67cbfc43000000000d008307?xsec_token=AB73VnYnGNx5y36EtnnZfGmAmS-6Wzv8WMuGpfwfkg6Yc%3D&xsec_source=pc_search) للتحديثات الرسمية. |
...
## لقطة قياس الأداء (RedClaw مقابل OpenClaw، قابلة للتكرار)
...
> ملاحظات: تم قياس نتائج RedClaw في إصدارات الإنتاج باستخدام `/usr/bin/time -l`. يتطلب OpenClaw وقت تشغيل Node.js (عادةً ~390 ميغابايت من عبء الذاكرة الإضافي)، بينما يتطلب NanoBot وقت تشغيل Python. PicoClaw و RedClaw هما ملفات ثنائية ثابتة. أرقام الذاكرة العشوائية أعلاه هي ذاكرة وقت التشغيل؛ متطلبات التجميع في وقت البناء أعلى.

<p align="center">
  <img src="zero-claw.jpeg" alt="مقارنة RedClaw مقابل OpenClaw" width="800" />
</p>
...
- `redclaw --help`: وقت حقيقي حوالي `0.02s`، بصمة ذاكرة قصوى ~`3.9 ميغابايت`
- `redclaw status`: وقت حقيقي حوالي `0.01s`، بصمة ذاكرة قصوى ~`4.1 ميغابايت`
...
يقوم نص `bootstrap.sh` بتثبيت Rust ونسخ RedClaw وتجميعه وإعداد بيئة التطوير الأولية الخاصة بك:
...
2. نسخ مستودع RedClaw
3. تجميع RedClaw في وضع الإصدار
4. تثبيت `redclaw` في `~/.cargo/bin/`
5. إنشاء هيكل مساحة العمل الافتراضية في `~/.redclaw/workspace/`
6. إنشاء ملف تكوين بدء التشغيل `~/.redclaw/workspace/config.toml`

بعد التمهيد، أعد تحميل shell الخاص بك أو قم بتشغيل `source ~/.cargo/env` لاستخدام أمر `redclaw` عالميًا.
...
1. قم بتكوين موفري الذكاء الاصطناعي الخاص بك في `~/.redclaw/workspace/config.toml`
2. تحقق من [مرجع التكوين](docs/config-reference.md) للخيارات المتقدمة
3. ابدأ الوكيل: `redclaw agent start`
4. اختبر عبر قناتك المفضلة (راجع [مرجع القنوات](docs/channels-reference.md))

## التكوين

قم بتحرير `~/.redclaw/workspace/config.toml` لتكوين الموفرون والقنوات وسلوك النظام.
...
يدعم RedClaw واجهتين خلفيتين لتنفيذ الكود:
...
```bash
# إدارة مساحة العمل
redclaw init                # تهيئة مساحة عمل جديدة
redclaw status              # عرض حالة البرنامج الخفي/الوكيل
redclaw config validate     # التحقق من بنية وقيم config.toml

# إدارة البرنامج الخفي
redclaw daemon start        # بدء البرنامج الخفي في الخلفية
redclaw daemon stop         # إيقاف البرنامج الخفي قيد التشغيل
redclaw daemon restart      # إعادة تشغيل البرنامج الخفي (إعادة تحميل التكوين)
redclaw daemon logs         # عرض سجلات البرنامج الخفي

# إدارة الوكيل
redclaw agent start         # بدء الوكيل (يتطلب تشغيل البرنامج الخفي)
redclaw agent stop          # إيقاف الوكيل
redclaw agent restart       # إعادة تشغيل الوكيل (إعادة تحميل التكوين)

# عمليات الاقتران
redclaw pairing init        # إنشاء سر اقتران جديد
redclaw pairing rotate      # تدوير سر الاقتران الحالي

# الأنفاق (للتعرض العام)
redclaw tunnel start        # بدء نفق إلى البرنامج الخفي المحلي
redclaw tunnel stop         # إيقاف النفق النشط

# التشخيص
redclaw doctor              # تشغيل فحوصات صحة النظام
redclaw version             # عرض الإصدار ومعلومات البناء
```
...
ادعُ `@redclaw:matrix.org` إلى غرفة مشفرة، وسيستجيب البوت بتشفير كامل. راجع [دليل Matrix E2EE](docs/matrix-e2ee-guide.md) لإعداد التحقق من الجهاز.
...
يدعم RedClaw نقاط النهاية المتوافقة مع OpenAI:
...
يوفر RedClaw أدوات مدمجة لتنفيذ الكود والوصول إلى نظام الملفات واسترجاع الويب:
...
تم تصميم RedClaw للعمل على أجهزة منخفضة الطاقة:
...
اعرض البرنامج الخفي RedClaw المحلي الخاص بك للشبكة العامة عبر أنفاق آمنة:
...
ينفذ RedClaw طبقات متعددة من الأمان:

### الاقتران

يُنشئ البرنامج الخفي سر اقتران عند التشغيل الأول مخزن في `~/.redclaw/workspace/.pairing`. يجب على العملاء (الوكيل، CLI) تقديم هذا السر للاتصال.

```bash
redclaw pairing rotate  # يُنشئ سرًا جديدًا ويبطل القديم
```
...
يسجل RedClaw في `~/.redclaw/workspace/logs/` افتراضيًا. يتم تخزين السجلات حسب المكون:
...
دعم مقاييس Prometheus لمراقبة الإنتاج قريبًا. التتبع في [#234](https://github.com/redclaw-labs/redclaw/issues/234).

## المهارات

يدعم RedClaw المهارات المخصصة — وحدات قابلة لإعادة الاستخدام توسع قدرات النظام.

### تعريف المهارة

يتم تخزين المهارات في `~/.redclaw/workspace/skills/<skill-name>/` بهذا الهيكل:
...
يدعم RedClaw [Open Skills](https://github.com/openagents-com/open-skills) — نظام معياري ومحايد للمورد لتوسيع قدرات وكلاء الذكاء الاصطناعي.
...
يمكنك أيضًا التجاوز في وقت التشغيل باستخدام `REDCLAW_OPEN_SKILLS_ENABLED` و `REDCLAW_OPEN_SKILLS_DIR`.
...
تم تكوين RedClaw لاستخدام `rustls` لتبعيات HTTP/TLS؛ `--locked` يحافظ على الرسم البياني العابر حتمي في البيئات النظيفة.
...
إذا كان RedClaw يساعد عملك وترغب في دعم التطوير المستمر، يمكنك التبرع هنا:
...
**هذا هو مستودع RedClaw الرسمي الوحيد:**

> <https://github.com/redclaw-labs/redclaw>

أي مستودع أو منظمة أو نطاق أو حزمة آخر يدعي أنه "RedClaw" أو يلمح إلى الارتباط بـ RedClaw Labs هو **غير مصرح به وغير مرتبط بهذا المشروع**. سيتم إدراج الفروع غير المصرح بها المعروفة في [TRADEMARK.md](TRADEMARK.md).

إذا واجهت انتحالًا أو سوء استخدام للعلامة التجارية، يرجى [فتح مشكلة](https://github.com/redclaw-labs/redclaw/issues).

---

## الترخيص

RedClaw مرخص بشكل مزدوج لأقصى قدر من الانفتاح وحماية المساهمين:
...
اسم **RedClaw** والشعار علامتان تجاريتان مسجلتان لـ RedClaw Labs. لا يمنح هذا الترخيص الإذن باستخدامهما للإيحاء بالموافقة أو الارتباط. راجع [TRADEMARK.md](TRADEMARK.md) للاستخدامات المسموح بها والمحظورة.
...
- `Skill` جديدة ← `~/.redclaw/workspace/skills/<n>/`

---

**RedClaw** — صفر عبء. صفر تنازلات. انشر في أي مكان. استبدل أي شيء. 🦀
...
<p align="center">
  <a href="https://www.star-history.com/#redclaw-labs/redclaw&type=date&legend=top-left">
    <picture>
     ...
     <img alt="رسم بياني لتاريخ النجوم" src="https://api.star-history.com/svg?repos=redclaw-labs/redclaw&type=date&legend=top-left" />
    </picture>
  </a>
</p>
