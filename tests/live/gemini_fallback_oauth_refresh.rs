...
//!
//! Requires:
//! - Live Gemini OAuth profile in `~/.redclaw/auth-profiles.json` with refresh_token
...
async fn gemini_warmup_refreshes_expired_oauth_token() -> Result<()> {
    // Find ~/.redclaw/auth-profiles.json
    let home = env::var("HOME").expect("HOME env var not set");
    let redclaw_dir = PathBuf::from(home).join(".redclaw");
    let auth_profiles_path = redclaw_dir.join("auth-profiles.json");
    ...
    if !auth_profiles_path.exists() {
        ...
        eprintln!("Run: redclaw auth login --provider gemini");
        return Ok(());
    }
    ...
    let gemini_profile_key = profiles
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "No Gemini OAuth profile found. Run: redclaw auth login --provider gemini"
            )
        })?
        .clone();
    ...
    // This will load auth from ~/.redclaw/auth-profiles.json (with expired token)
